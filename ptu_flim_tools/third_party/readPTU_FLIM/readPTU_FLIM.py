...
def get_flim_data_stack_static(
    sync, tcspc, channel, special, header_variables
):
    ...
    if (ImgHdr_Ident == 9) or (ImgHdr_Ident == 3):
        ...
        read_data_range = np.where(sync == FrameSyncVal[num_of_Frames - 1])[0][
            0
        ]
        ...
        for event in range(read_data_range + 1):
            ...
            if isPhoton and insideLine and insideFrame:
                currentPixel = int(
                    np.floor(
                        (
                            ((currentSync - syncStart) / syncPulsesPerLine)
                            * num_pixel_X
                        )
                    )
                )
                ...
                if (currentPixel < num_pixel_X) and (
                    tmptcspc < num_tcspc_channel
                ):
                    flim_data_stack[currentLine][currentPixel][tmpchan][
                        tmptcspc
                    ] += 1
    #         else:
...
class PTUreader:
    ...
    def __init__(self, filename, print_header_data=False):
        ...
        self.ptu_data_string = (
            f.read()
        )  # ptu_data_string is a string of bytes and reads all file in memory
        f.close()
    ...
    def _ptu_read_tags(self, ptu_data_string, offset):
        ...
        tag_struct = struct.unpack(
            "32s i I q", ptu_data_string[offset : offset + 48]
        )
        offset += 48
        ...
        if tag["type"] == "tyAnsiString":
            try:
                tag["data"] = (
                    ptu_data_string[offset : offset + tag["value"]]
                    .rstrip(b"\0")
                    .decode()
                )
            ...
            tag["data"] = np.frombuffer(
                ptu_data_string, dtype="float", count=tag["value"] / 8
            )
            offset += tag["value"]
        elif tag["type"] == "tyWideString":
            # WideString default encoding is UTF-16.
            tag["data"] = ptu_data_string[
                offset : offset + tag["value"] * 2
            ].decode("utf16")
            tagStringR = tag["data"]
            offset += tag["value"]
    ...
    def _ptu_read_head(self, ptu_data_string):
        ...
        tagName, tagValue, offset, tagString = self._ptu_read_tags(
            ptu_data_string, offset
        )
    ...
    def _ptu_read_raw_data(self):
        ...
        if record_type == "rtPicoHarpT3":
            ...
            tcspc = np.bitwise_and(
                np.right_shift(t3records, 16), 4095
            )  # Next 12 bits, dtime can be obtained from header
            chan = np.bitwise_and(
                np.right_shift(t3records, 28), 15
            )  # Next 4 bits
            special = ((chan == 15) * 1) * (
            ...
            tcspc = np.bitwise_and(
                t3records, 15
            )  # Next 4 bits, dtime can be obtained from header
            chan = np.bitwise_and(
                np.right_shift(t3records, 28), 15
            )  # Next 4 bits
            special = ((chan == 15) * 1) * np.bitwise_and(
                tcspc, 15
            )  # Last bit for special markers
            index = ((chan == 15) * 1) * (
                (np.bitwise_and(tcspc, 15) == 0) * 1
            ...
            tcspc = np.bitwise_and(
                np.right_shift(t3records, 10), 32767
            )  # Next 15 bits, dtime can be obtained from header
            chan = np.bitwise_and(
                np.right_shift(t3records, 25), 63
            )  # Next 8 bits
            special = (
                np.bitwise_and(t3records, 2147483648) > 0
            )  # Last bit for special markers
            index = (special * 1) * (
                (chan == 63) * 1
            )  # Find overflow locations
            ...
            special = np.bitwise_and(
                np.right_shift(t3records, 31), 1
            )  # Last bit for special markers
            index = (special * 1) * (
                (chan == 63) * 1
            )  # Find overflow locations
        ...
        if record_type in [
            ...
            sync = sync + (
                WRAPAROUND * np.cumsum(index * sync)
            )  # For HydraHarp V1 and TimeHarp260 V1 overflow corrections
        else:
            sync = sync + (
                WRAPAROUND * np.cumsum(index)
    ...
    def get_flim_data_stack(self):
        ...
        if self.head["Measurement_SubMode"] == 0:
            ...
            raise IOError(
                "Piezo Scanner Data Reader Not Implemented Yet!!! \n"
            )
            sys.exit()
        else:
        ...
        flim_data_stack = get_flim_data_stack_static(
            sync, tcspc, channel, special, header_variables
        )

        if flim_data_stack.ndim == 4:
            tmp_intensity_image = np.sum(
                flim_data_stack, axis=3
            )  # sum across tcspc bin
            intensity_image = np.sum(
                tmp_intensity_image, axis=2
            )  # sum across spectral channels

        elif flim_data_stack.ndim == 3:
            intensity_image = np.sum(
                flim_data_stack, axis=3
            )  # sum across tcspc bin, only 1 detection channel

        return flim_data_stack, intensity_image


def get_lifetime_image(
    flim_data_stack,
    channel_number,
    timegating_start1,
    timegating_stop1,
    ...
    work_data = flim_data_stack[
        :, :, channel_number, timegating_start1:timegating_stop1
    ]
    bin_range = np.reshape(
        np.linspace(0, timegating_stop1, timegating_stop1),
        (1, 1, timegating_stop1),
    )

    fast_flim = (
        np.sum(work_data * bin_range, axis=2) * meas_resolution
    ) / np.sum(work_data, axis=2)
